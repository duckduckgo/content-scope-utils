  ...
  async wkSendAndWait(handler, data = {}) {
    ...
    try {
      ...
      const { ciphertext, tag } = await new this.globals.Promise((/** @type {any} */ resolve) => {
        ...
        data.messageHandling = SecureMessagingParams.from({
 ...
 *
 * const config = WebkitMessagingConfig.from({
 *   hasModernWebkitAPI: true,
 *   webkitMessageHandlerNames: ["foo", "bar", "baz"],
 *   secret: "dax",
 * });
...
export class WebkitMessagingConfig {
  ...
  /**
   * A helper for constructing `WebkitMessagingConfig` from anything
   * with enough matching fields
   * @template {WebkitMessagingConfig} T
   * @param {T} params
   * @returns {WebkitMessagingConfig}
   */
  static from(params) {
    return new WebkitMessagingConfig(
      params.hasModernWebkitAPI,
      params.webkitMessageHandlerNames.slice(),
      params.secret
    );
  }
}
...
export class SecureMessagingParams {
   ...
   * @param {number[]} iv
   */
  constructor(methodName, secret, key, iv) {
    /**
  ...
  /**
   * A convenience for type-checking
   * @template {SecureMessagingParams} T
   * @param {T} params
   * @returns {SecureMessagingParams}
   */
  static from(params) {
    return params;
  }
