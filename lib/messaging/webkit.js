  ...
  wkSend(handler, data = {}) {
    ...
    if (!this.config.hasModernWebkitAPI) {
      if (!(handler in ddgGlobals.capturedWebkitHandlers)) {
        throw new Error(
          `cannot continue, method ${handler} not captured on macos < 11`
        );
      } else {
...
const createRandKey = async () => {
  const key = await ddgGlobals.generateKey(algoObj, true, [
    "encrypt",
    "decrypt",
  ]);
 ...
 */
const createRandIv = () =>
  ddgGlobals.getRandomValues(new ddgGlobals.Uint8Array(12));
...
const decrypt = async (ciphertext, key, iv) => {
  const cryptoKey = await ddgGlobals.importKey("raw", key, "AES-GCM", false, [
    "decrypt",
  ]);
...
function captureWebkitHandlers(handlerNames) {
  for (let webkitMessageHandlerName of handlerNames) {
    if (
      typeof window.webkit.messageHandlers?.[webkitMessageHandlerName]
       ...
       */
      // @ts-ignore
      ddgGlobals.capturedWebkitHandlers[webkitMessageHandlerName] =
        window.webkit.messageHandlers[
          webkitMessageHandlerName
        ].postMessage?.bind(
          window.webkit.messageHandlers[webkitMessageHandlerName]
        );
      delete window.webkit.messageHandlers[webkitMessageHandlerName]
        .postMessage;
    }
  }
}
